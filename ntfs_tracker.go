...
// to the byte stream path, where the actual bytes are available.
...
type NTFSTracker struct {
	...
	/*
		NTFS data may be written before the corresponding MFT update tells
		us which file owns it.

		OnMetadataWrite discovers new candidate ranges. We queue those
		ranges here so DiskNode can replay the already-persisted temporary
		bytes through the TS detector.
	*/
	pendingCandidates []ByteRange

	/*
		These identify genuinely new/reused file streams.

		A new recording does not necessarily first become visible at
		logical file offset 0. Therefore DiskNode needs an explicit
		"this stream is new" signal.
	*/
	pendingNewStreams map[string]struct{}
}
...
func (t *NTFSTracker) Classify(
	...
	/*
		Do not use the generic coalescer here.

		NTFS candidate ranges carry logical StreamOffset information.
		Two physically adjacent ranges are only safely merged when their
		logical stream offsets are adjacent too.
	*/
	return coalesceClassifiedRanges(out)
}

/*
coalesceClassifiedRanges preserves the logical stream mapping carried by
candidate ranges.
*/
func coalesceClassifiedRanges(
	in []ByteRange,
) []ByteRange {
	if len(in) == 0 {
		return nil
...
/*
OnMetadataWrite refreshes MFT information after metadata has been persisted.

The important delayed-discovery behavior is:

	physical payload write
	    ↓
	MFT does not know the extent yet
	    ↓
	DiskNode temporarily stores it
	    ↓
	MFT is updated
	    ↓
	this function discovers the new data extent
	    ↓
	the range is queued for replay
*/
func (t *NTFSTracker) OnMetadataWrite(
	start,
	length uint64,
) {
	if length == 0 {
	...
	oldStreams := t.activeStreamStatesLocked()

	ids, touchesRecordZero :=
		t.mftRecordIDsForPhysicalRangeLocked(
			start,
			end,
		)
	...
	/*
		Record 0 contains $MFT's own runlist.

		Refresh it first so a volume whose MFT grows gets the current
		mapping before we inspect newly allocated records.
	*/
	...
	if t.mftSize > oldMFTSize {
		if err := t.scanMFTLogicalRangeLocked(
			oldMFTSize,
			t.mftSize,
		); err != nil {
			if verbLog {
				log.Printf(
					"NTFS: scan newly grown MFT range failed: %v",
					err,
				)
			}
		} else {
	...
	for id := range ids {
		...
		if err := t.refreshRecordLocked(id); err != nil {
			/*
				A recorder may update an MFT record through a write that
				is observed while the record is still in an incomplete
				multi-sector state.

				Preserve the previous snapshot and retry later.
			*/
			if verbLog {
				log.Printf(
	...
	/*
		A shrinking MFT is unusual, but pruning stale record snapshots
	...
	*/
	...
	/*
		...
		1. physical file-data ranges that were not previously visible
		2. file streams which represent a genuinely new/reused MFT file
	*/
	newRanges := subtractNTSRanges(
		t.dataRanges,
		oldRanges,
	)
	...
	newStreams :=
		newlyDiscoveredStreams(
			oldStreams,
			t.activeStreamStatesLocked(),
		)

	for streamID := range newStreams {
		t.pendingNewStreams[streamID] = struct{}{}
	}

	if verbLog {
		if len(newRanges) > 0 {
			log.Printf(
				"NTFS: discovered %d new candidate range(s)",
				len(newRanges),
			)
		}
...
/*
DrainDiscovery returns and clears the delayed NTFS discovery queue.

The Tracker interface itself stays unchanged. DiskNode type-asserts the
concrete NTFSTracker and calls this method after OnMetadataWrite().
*/
func (t *NTFSTracker) DrainDiscovery() (
	[]ByteRange,
	map[string]struct{},
) {
	t.mu.Lock()
	...
	sort.Slice(ranges, func(i, j int) bool {
		...
		if ranges[i].StreamOffset != ranges[j].StreamOffset {
			return ranges[i].StreamOffset < ranges[j].StreamOffset
		}
	...
	streams := make(
		map[string]struct{},
		len(t.pendingNewStreams),
	)

	for streamID := range t.pendingNewStreams {
		streams[streamID] = struct{}{}
	}

	t.pendingCandidates = nil
	t.pendingNewStreams = make(
		map[string]struct{},
	)

	return ranges, streams
}

func (t *NTFSTracker) readBootSector() error {
	var boot [512]byte

	if _, err := t.reader.ReadAt(
		boot[:],
		t.partition.Offset,
	); err != nil {
		return fmt.Errorf(
	...
	if bps == 0 || spcByte == 0 {
		return fmt.Errorf(
			"invalid NTFS sector/cluster geometry: bps=%d spc=%d",
			bps,
			spcByte,
		)
	}
	...
	if spc > 128 ||
		bps > ^uint64(0)/spc {
		return fmt.Errorf(
			"unsupported NTFS sectors/cluster %d",
			spc,
		)
	}
	...
	t.bootMFTLCN = binary.LittleEndian.Uint64(
		boot[48:56],
	)

	recordSize, err := decodeNTFSRecordSize(
		int8(boot[64]),
		spc,
		bps,
	)
	...
	if partClusters == 0 {
		return fmt.Errorf(
			"NTFS partition contains no clusters",
		)
	}

	if t.bootMFTLCN >= partClusters {
		return fmt.Errorf(
			"MFT LCN %d outside volume",
			t.bootMFTLCN,
		)
	}
...
func decodeNTFSRecordSize(
	...
	if v > 0 {
		if uint64(v) >
			^uint64(0)/sectorsPerCluster {
			return 0, fmt.Errorf(
				"MFT record cluster count overflow",
			)
		}
		...
		if clusters >
			^uint64(0)/bytesPerSector {
			return 0, fmt.Errorf(
				"MFT record size overflow",
			)
		}
	...
	if shift == 0 || shift >= 63 {
		return 0, fmt.Errorf(
			"MFT record-size exponent too large: %d",
			shift,
		)
	}
...
func (t *NTFSTracker) loadMFTLayout() error {
	/*
		$MFT record 0 starts at the LCN advertised by the boot sector.
		Its first record is necessarily readable before we know the
		complete MFT runlist.
	*/
	off :=
		uint64(t.partition.Offset) +
			t.bootMFTLCN*t.clusterSize
	...
	if data.Flags&(ntfsAttrCompressed|ntfsAttrEncrypted|ntfsAttrSparse) != 0 {
		return fmt.Errorf(
			"$MFT $DATA has unsupported flags %#x",
			data.Flags,
		)
	}
...
/*
applyNTFSFixup applies the Update Sequence Array using the actual NTFS
bytes-per-sector geometry.

The previous implementation hardcoded 512-byte sectors. NTFS records are
protected sector-by-sector, so the boot-sector bytes-per-sector value is the
correct stride.
*/
func applyNTFSFixup(
	record []byte,
	...
	sectors := len(record) / stride

	if usaOff < 8 ||
		usaCount != sectors+1 ||
		usaOff+usaCount*2 > len(record) {
		return fmt.Errorf(
...
func parseNTFSAttribute(
	...
	if nameLen > 0 {
		...
		/*
			We deliberately do not decode or inspect the name.

			Unnamed $DATA is the default file stream. Named data streams
			are not candidates.
		*/
		a.Name = "<named>"
	}
	...
	if !a.NonResident {
		...
		a.ValueLen = binary.LittleEndian.Uint32(
			record[off+16 : off+20],
		)

		a.ValueOff = binary.LittleEndian.Uint16(
			record[off+20 : off+22],
		)
		...
		valueEnd :=
			valueStart + int(a.ValueLen)

		if valueStart < off ||
			valueEnd < valueStart ||
			valueEnd > attrEnd {
			return a, fmt.Errorf(
				"invalid NTFS resident attribute value",
			)
		}
	...
	a.MappingPairsOff = binary.LittleEndian.Uint16(
	...
	if pairsStart == attrEnd {
		if a.RealSize != 0 {
			return a, fmt.Errorf(
				"nonresident NTFS attribute has no mapping pairs",
			)
		}
	...
	if len(runs) == 0 &&
		a.RealSize != 0 {
		return a, fmt.Errorf(
			"nonresident NTFS attribute has empty runlist",
		)
	}

	if len(runs) > 0 &&
		a.LastVCN < a.StartingVCN {
		return a, fmt.Errorf(
			"invalid NTFS VCN range",
		)
	}
...
func parseNTFSRecord(
	buf []byte,
	id uint64,
) (*ntfsRecordInfo, []ntfsAttribute, error) {
	if len(buf) < 48 ||
	...
	if firstAttr < 0x18 ||
		firstAttr >= len(buf) {
		return nil, nil, fmt.Errorf(
			"invalid first attribute offset %d",
			firstAttr,
		)
	}
...
func (t *NTFSTracker) readRawRecordByOffset(
	off uint64,
) ([]byte, error) {
...
*/
func (t *NTFSTracker) readMFTLogical(
	logical uint64,
	dst []byte,
) error {
	if len(dst) == 0 {
	...
	if logical > t.mftSize ||
		uint64(len(dst)) > t.mftSize-logical {
		return fmt.Errorf(
			"MFT logical read outside $MFT size",
		)
	}
	...
	for remaining > 0 {
		...
		for _, run := range t.mftRuns {
			...
			if uint64(run.LCN) >
				(^uint64(0)-uint64(t.partition.Offset))/
					t.clusterSize {
				return fmt.Errorf(
					"MFT physical offset overflow",
				)
			}

			phys :=
				uint64(t.partition.Offset) +
					uint64(run.LCN)*t.clusterSize +
					within

			if available >
				uint64(^uint(0)>>1) {
				return fmt.Errorf(
					"MFT physical read too large",
				)
			}

			if _, err :=
				t.reader.ReadAt(
					dst[int(dstPos):int(dstPos+available)],
					int64(phys),
				); err != nil {
				return err
			}
		...
		if !mapped {
			return fmt.Errorf(
				"MFT logical offset %d is not mapped",
				logicalPos,
			)
		}
	}
...
func (t *NTFSTracker) mftRecordIDsForPhysicalRangeLocked(
	start,
	end uint64,
...
func (t *NTFSTracker) physicalRangesForMFTLogical(
	logical,
	length uint64,
	kind RangeKind,
) []ByteRange {
...
func (t *NTFSTracker) refreshRecordLocked(
	id uint64,
) error {
	info, attrs, err :=
		t.readRecord(id)
...
func (t *NTFSTracker) buildDataRanges(
	info *ntfsRecordInfo,
	attrs []ntfsAttribute,
) {
	info.DataRanges = nil
	...
	/*
		Only user files are candidates.

		Extension records are accepted when their BaseID points to a
		user file. rebuildRangesLocked verifies that the base record is
		live and a regular file before exposing the ranges.
	*/
	...
	for _, a := range attrs {
		...
		for _, run := range a.Runs {
			if run.Sparse ||
				run.LCN < 0 ||
				uint64(run.LCN) >= t.totalClusters {
				continue
			}
...
func (t *NTFSTracker) buildRootRanges(
	info *ntfsRecordInfo,
	attrs []ntfsAttribute,
) {
	info.RootMetaRange = nil

	if info.ID != ntfsRootRecord ||
		!info.InUse {
		return
	}

	/*
		The root record itself contains resident $INDEX_ROOT and other
...
func (t *NTFSTracker) scanMFT() error {
	...
	if count == 0 {
		return fmt.Errorf(
			"partition cannot contain an MFT record",
		)
	}
	...
	t.mftSize =
		minU64(
			t.mftSize,
			count*t.mftRecordSize,
		)

	if t.mftSize < t.mftRecordSize {
		return fmt.Errorf(
			"$MFT is smaller than one record",
		)
	}
...
func (t *NTFSTracker) scanMFTLogicalRangeLocked(
	logicalStart,
	logicalEnd uint64,
) error {
	if logicalStart > logicalEnd ||
		logicalEnd > t.mftSize {
		return fmt.Errorf(
			"invalid MFT scan range %#x-%#x",
			logicalStart,
			logicalEnd,
		)
	}
	...
	for logical := logicalStart; logical < logicalEnd; {
		...
		if verbLog {
			...
			log.Printf(
				"NTFS: scanned MFT record %d/%d",
				minU64(current, count),
				count,
			)
		}
	}
...
/*
activeStreamStatesLocked determines the current logical recording-file
instances.

The sequence number matters because NTFS can reuse an MFT record number for
another file; the sequence number changes specifically to prevent stale
references from being mistaken for the old file.

HasData is aggregated across the base record and its extension records.
*/
func (t *NTFSTracker) activeStreamStatesLocked() map[uint64]ntfsStreamState {
	states := make(
		map[uint64]ntfsStreamState,
	)

	/*
		First collect the live base records.
	*/
	for id, info := range t.records {
		...
		states[id] = ntfsStreamState{
			Sequence: info.Sequence,
		}
	}

	/*
		Then determine whether any record belonging to that base file
		contains candidate data.

		An extension record can contain part of the stream, so checking
		only the base record is insufficient.
	*/
	for _, info := range t.records {
		if !info.InUse ||
			info.BaseID < ntfsFirstUserRecord ||
			info.IsDir ||
			len(info.DataRanges) == 0 {
			continue
		}
		...
		state.HasData = true
		states[info.BaseID] = state
	}

	return states
}

func newlyDiscoveredStreams(
	oldStates,
	newStates map[uint64]ntfsStreamState,
) map[string]struct{} {
	out := make(
		map[string]struct{},
	)

	for baseID, current := range newStates {
		previous, existed :=
			oldStates[baseID]
		...
		/*
			Only queue streams that actually have file-data ranges.

			This avoids resetting the TS detector merely because an MFT
			record was created before its $DATA stream was populated.
		*/
		if !isNew || !current.HasData {
			continue
		}
...
/*
subtractNTSRanges computes candidate physical ranges present in newRanges
but not already represented by oldRanges for the same logical stream mapping.

A physical rewrite of an existing logical offset is considered new if it moved
to a different physical extent.
*/
func subtractNTSRanges(
	...
	sort.Slice(out, func(i, j int) bool {
		...
		if out[i].StreamOffset !=
			out[j].StreamOffset {
			return out[i].StreamOffset <
				out[j].StreamOffset
		}
...
func (t *NTFSTracker) rebuildRangesLocked() {
	...
	for id, info := range t.records {
		/*
			A base record identifies the file.

			Only ordinary, in-use files are allowed to activate their
			own and extension records' data ranges.
		*/
		if !info.InUse ||
			info.IsDir ||
			id != info.BaseID ||
	...
	sort.Slice(data, func(i, j int) bool {
		if data[i].Start == data[j].Start {
			if data[i].End == data[j].End {
				if data[i].StreamID ==
					data[j].StreamID {
					return data[i].StreamOffset <
						data[j].StreamOffset
				}

				return data[i].StreamID <
					data[j].StreamID
			}
